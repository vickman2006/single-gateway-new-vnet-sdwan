...
module "vnet" {
  ...
  subnet_prefixes     = [
                          var.frontend_1_subnet_prefix,
                          var.frontend_2_subnet_prefix,
                          var.backend_subnet_prefix
                        ]
  subnet_names        = [
                          "${var.single_gateway_name}-frontend-1-subnet",
                          "${var.single_gateway_name}-frontend-2-subnet",
                          "${var.single_gateway_name}-backend-subnet"
                        ]
...
/*------------------------------------------------
  NICs y Asociaciones de Seguridad
------------------------------------------------*/

/* NIC para frontend-1 (índice 0) → nombre: nic */
resource "azurerm_network_interface" "nic" {
  ...
  ip_configuration {
    ...
    private_ip_address            = cidrhost(var.frontend_1_subnet_prefix, 4)
    public_ip_address_id          = azurerm_public_ip.public-ip.id
  }
}

/* NIC para frontend-2 (índice 1) → nombre: nic1 */
resource "azurerm_network_interface" "nic1" {
  ...
  ip_configuration {
    ...
    private_ip_address_allocation = var.vnet_allocation_method
    private_ip_address            = cidrhost(var.frontend_2_subnet_prefix, 4)
  }
}

/* NIC para backend (índice 2) → nombre: nic2 */
...
/* Asociaciones NSG */
/* Asociar NSG a la NIC de frontend-1 */
...
/* Asociar NSG a la NIC de frontend-2 */
resource "azurerm_network_interface_security_group_association" "security_group_association_nic1" {
  depends_on                = [azurerm_network_interface.nic1, module.network-security-group]
...
resource "azurerm_virtual_machine" "single-gateway-vm-instance" {
  ...
  os_profile_linux_config {
    ...
    dynamic "ssh_keys" {
      for_each = local.SSH_authentication_type_condition ? [1] : []
      content {
