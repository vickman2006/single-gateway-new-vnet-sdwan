...
locals {
  ...
  // Will fail if var.frontend_1_subnet_prefix is invalid
  regex_frontend_1_subnet_prefix = regex(local.regex_valid_network_cidr, var.frontend_1_subnet_prefix) == var.frontend_1_subnet_prefix ? 0 : "Variable [frontend_1_subnet_prefix] must be a valid address in CIDR notation."
  
  // Will fail if var.frontend_2_subnet_prefix is invalid
  regex_frontend_2_subnet_prefix = regex(local.regex_valid_network_cidr, var.frontend_2_subnet_prefix) == var.frontend_2_subnet_prefix ? 0 : "Variable [frontend_2_subnet_prefix] must be a valid address in CIDR notation."
